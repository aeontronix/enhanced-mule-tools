...
public class ApplicationEnhancer {
    ...
    public static void enhanceApplicationArchive(ObjectMapper objectMapper, ApplicationSource source, ObjectNode applicationDescriptor, boolean deletePreEnhancedArchive) throws IOException, UnpackException {
        final File file = source.getLocalFile();
        try (final TempFile tempFile = new TempFile("enh", "zip")) {
            applicationDescriptor.put("processed", true);
            Unpacker unpacker = new Unpacker(file, FileType.ZIP, tempFile, FileType.ZIP);
            ...
            transformers.add(new Transformer(new FileMatcher(ANYPOINT_JSON, CREATE)) {
                ...
                public byte[] transform(SourceFile sourceFile) throws Exception {
                    return objectMapper.writeValueAsBytes(applicationDescriptor);
                }
            });
            unpacker.addTransformers(transformers);
            unpacker.unpack();
            source.replaceFile(tempFile);
        }
    }
