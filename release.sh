...
if [[ "${POM_REL_VERSION}" =~ .*"beta".* ]] || [[ "${POM_REL_VERSION}" =~ .*"alpha".* ]]; then
  ...
  curl --header 'Content-Type: application/json' --header "PRIVATE-TOKEN: ${GL_TOKEN}" \
     --data "{ \"name\": \"v${POM_REL_VERSION}\", \"tag_name\": \"v${POM_REL_VERSION}\", \"description\": \"Release v${POM_REL_VERSION}\", \"assets\": { \"links\": [{ \"name\": \"CLI Distribution (ZIP)\", \"url\": \"https://repo1.maven.org/maven2/com/aeontronix/enhanced-mule/enhanced-mule-tools-cli/${POM_REL_VERSION}/enhanced-mule-tools-cli-${POM_REL_VERSION}-dist.zip\" },{ \"name\": \"CLI Windows executable\", \"url\": \"https://repo1.maven.org/maven2/com/aeontronix/enhanced-mule/enhanced-mule-tools-cli/${POM_REL_VERSION}/enhanced-mule-tools-cli-${POM_REL_VERSION}.exe\" }] } }" \
